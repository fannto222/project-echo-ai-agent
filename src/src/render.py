...
def _caption_chunks(text: str, portrait: bool) -> list[str]:
    """Create short, readable caption bursts instead of sentence-sized subtitles."""
...
def make_srt(text: str, total_seconds: float, out: Path, portrait: bool) -> None:
    ...
    for i, (phrase, w) in enumerate(zip(items, weights), start=1):
        ...
        # Prevent ultra-fast flicker but still keep Shorts snappy.
        if portrait:
            seg = max(0.65, seg)
    ...
    # Ensure the final caption reaches the exact end of the voice track.
...
def render_video(clips: list[Path], voice: Path, narration: str, out: Path, portrait: bool) -> None:
    ...
    # Shorts need much faster visual turnover than long-form.
    if portrait:
        target_scene = 2.4
        segment_len = min(3.0, max(1.8, voice_dur / max(1, len(clips))))
        # Never let the adaptive value become slower than the Shorts target.
        segment_len = min(segment_len, target_scene)
    else:
    ...
    for i, clip in enumerate(sequence):
        ...
        # Start later on repeated uses of the same stock clip when possible.
        seek = (i % 3) * 0.65
        run([
    ...
    if portrait:
        # Lower-third captions, large enough for phones but no longer dominating the frame.
        font_size = 26
        margin_v = 245
        style = (
            "FontName=DejaVu Sans,FontSize=26,Bold=-1,"
            "PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,"
            "BorderStyle=1,Outline=3,Shadow=0,Alignment=2,MarginL=70,MarginR=70,MarginV=245"
        )
    else:
        font_size = 24
        margin_v = 55
        style = (
            ...
            "PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,"
            f"BorderStyle=1,Outline=3,Shadow=0,Alignment=2,MarginL=60,MarginR=60,MarginV={margin_v}"
        )
